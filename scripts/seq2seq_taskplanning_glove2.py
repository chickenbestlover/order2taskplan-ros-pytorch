...
def closest2(d,n=10):
    t1=time.time()
    all_dists = [(w, torch.mm(d.unsqueeze(0), get_word(w).unsqueeze(0).t())[0][0]) for w in wv_dict]
...
class Lang_glove:
    def __init__(self, name,wv_dict,wv_arr):
        self.name = name
        self.word2index = wv_dict
        #self.word2count = {}
        self.index2word = wv_index2dict
        self.n_words = len(wv_dict)  # Count SOS and EOS
...
def read_langs(lang1, lang2, lang3):
    ...
    input_lang1 = Lang_glove(lang1,wv_dict,wv_arr)
    input_lang2 = Lang_glove(lang2,wv_dict,wv_arr)
    output_lang = Lang_glove(lang3,wv_dict,wv_arr)
...
class EncoderRNN(nn.Module):
    def __init__(self, input_size=len(wv_dict), hidden_size=300, wv_dict=wv_dict, wv_arr=wv_arr,  n_layers=1):
        ...
        embedding_param=list(self.embedding.parameters())
        embedding_param[0].data=wv_arr
...
encoder1 = EncoderRNN(len(wv_dict), hidden_size, n_layers=n_layers)
encoder2 = EncoderRNN(len(wv_dict), hidden_size, n_layers=n_layers)
...
if not args.pretrained:
    for epoch in range(args.start_epoch, args.epochs+1):
        ...
        if epoch % print_every == 0:
            ...
            time_since(start, float(epoch) / args.epochs), epoch, float(epoch) / args.epochs * 100, print_loss_avg,print_loss_test_avg)
            print(print_summary)
...
def taskplan_filter(str):
    str = str.replace("arc toy", "a")
    str = str.replace("square toy", "s")
    str = str.replace("cylinder toy", "c")
    str = str.replace("triangle toy", "t")

    str = str.replace("blue ", "b")
    str = str.replace("green ", "g")
    str = str.replace("red ", "r")

    str = str.replace("toy box A", "box_a")
    str = str.replace("toy box B", "box_b")
    str = str.replace("toy box C", "box_c")

    str = str.replace("location 1", "loc_1")
    str = str.replace("location 2", "loc_2")

    return str

def publishTaskplan():
    pub = rospy.Publisher('taskplanPublisher', String, queue_size=10)
    rospy.init_node('order2taskplan', anonymous=True)
    rate = rospy.Rate(10)  # 10hz
    while not rospy.is_shutdown():
        pair = random.choice(pairs)
        taskplan = taskplan_filter(pair[2])
        rospy.loginfo(taskplan)
        pub.publish(taskplan)
        rate.sleep()
